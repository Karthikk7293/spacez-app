    ...
    return (
        <div className="z-50">
            ...
            <div className={` ${focus === name ? "visible" : "hidden"} duration-200 bg-white z-50 w-full flex justify-end gap-3`}>
                <button className=" text-xs cursor-pointer  px-3 py-2 underline underline-offset-2 active:bg-gray-100">Cancel</button>
                <button onClick={(() => handleSubmit())} className={` ${focus === name ? "active:bg-gray-100" : ""} border-gray-300 text-xs border cursor-pointer rounded-sm px-3 py-2   `} >Save</button>
            </div>
        </div>
    )
}
